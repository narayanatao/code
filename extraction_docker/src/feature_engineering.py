...
def correctLineRows(df):
    ...
    # df.to_csv("d://ftrs_bfr_corr.csv",index = False)
    try:
        ...
        # df.to_csv("d://ftrs_aft_corr.csv",index = False)
        return df
    except:
...
def addLineItemFeatures_New(DF):
    ...
    try:
        ...
        # d = {}
        # for ind, row in DF_hdr.itertuples():
        #     cond = (DF["page_num"] == row.page_num)
        #     cond = cond & (DF["tableLineNo"] == row.tableLineNo)
        #     tkns = list(DF[cond]["token_id"])
        #     d.update({tkn:row.score for tkn in tkns})
        # DF["score_new"] = DF["token_id"].map(d)
        # DF["score"] = np.where(DF["score_new"].isnull(),
        #                        DF["score"],DF["score_new"])

        # DF["score_1"] = 0
        # for ind, row in DF_hdr_1.itertuples():
        #     DF.loc[(DF["page_num"] == row.page_num) &
        #            (DF["tableLineNo"] == row.tableLineNo),
        #            "score_1"] = row.score
            ...
            if len(pot_lines) > 0:
                ...
                for line_no in pot_lines:
                    ...
                    for i,row in DF_line.iterrows():
                        ...
                        if matches:
                            # print("matched Text",line_text,matches)
                            found = True
                            if line_no in line_noanchor:
                                line_noanchor.remove(line_no)
                            break
                ...
                # print("Potential lines",pot_reduced_lines)
                ngbrs = []
                for line_no in pot_reduced_lines:
                    DF_line = DF[DF['tableLineNo'] == line_no]
                    neighbour = max(list(DF_line["noNeighbours"]))
...
def extract_max_amount_features_new(DF):
    ...
    try:
        ...
        print("extract_max_amount_features_new",
              traceback.print_exc())
        return df_copy
...
def populate_max_probableGST_amount(DF):
    ...
    try:
        ...
        TEMP = TEMP[['token_id', 'page_num', 'extracted_amount']]
        ...
        if len(l) > 0:
            ...
            max_amount = int(l[0])
            DF.loc[((DF['PROBABLE_GST_AMOUNT_SLAB'] == 1) & (DF['extracted_amount'] >= max_amount)), 'PROBABLE_GST_MAX'] = 1
        ...
        if len(l) > 1:
            ...
            DF.loc[((DF['PROBABLE_GST_AMOUNT_SLAB'] == 1) &
                    ...
                    (DF['extracted_amount'] >= second_max_amount)), 'PROBABLE_GST_SECOND_MAX'] = 1
...
def fuzzyMatchNgbrs(df,orientation,subject):
    ...
    try:
        ...
        print("FuzzyMatchNgbrs",traceback.print_exc())
        return df_copy
...
def extract_features(docInput):
    ...
    try:
        ...
        DFs = res.get()
        pool.terminate()
        print("process_df_features",time.time() - t)
        ...
        blob_ftr_file_path = os.path.join(ROOT_FOLDER,
                                          ...
                                          blob_ftr_file_name)
        if os.path.exists("d:"):
            DF_All.to_csv("d:/ftrs.csv",index = False)
...
if __name__ == "__main__":
    inp = {
           "documentId":"doc_1651739719017_99b8b999ab9",
           "client_folder":"Pierian",
           "container":"f3cce063-3d5b-4cec-ab48-623a853f859c",
           "page_details":
               [
                   {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-0-pre.tiff",
                    "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-0-pre.tiff.ocr.json",
                     "document_page_number":"0"},
                    {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-1-pre.tiff",
                     "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-1-pre.tiff.ocr.json",
                     "document_page_number":"1"},
                    {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-2-pre.tiff",
                     "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1651739719017_99b8b999ab9-2-pre.tiff.ocr.json",
                     "document_page_number":"2"}
                   # {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649825557228_898b9b98999-0-pre.tiff",
                   #  "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649825557228_898b9b98999-0-pre.tiff.ocr.json",
                   #  "document_page_number":"0"
                     #}#,
                   # {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-1-pre.tiff",
                   #  "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-1-pre.tiff.ocr.json",
                   #  "document_page_number":"1"
                   #    },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-2-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-2-pre.tiff.ocr.json",
                   #   "document_page_number":"2"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-3-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-3-pre.tiff.ocr.json",
                   #   "document_page_number":"3"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-4-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-4-pre.tiff.ocr.json",
                   #   "document_page_number":"4"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-5-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-5-pre.tiff.ocr.json",
                   #   "document_page_number":"5"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-6-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-6-pre.tiff.ocr.json",
                   #   "document_page_number":"6"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-7-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-7-pre.tiff.ocr.json",
                   #   "document_page_number":"7"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-8-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-8-pre.tiff.ocr.json",
                   #   "document_page_number":"8"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-9-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-9-pre.tiff.ocr.json",
                   #   "document_page_number":"9"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-10-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-10-pre.tiff.ocr.json",
                   #   "document_page_number":"10"
                   #   },
                   #  {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-11-pre.tiff",
                   #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1649183036407_ab9aa89a99b-11-pre.tiff.ocr.json",
