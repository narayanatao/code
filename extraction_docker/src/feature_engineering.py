...
def correctLineRowsFzScore(df):
    ...
    try:
        ...
        df["tbl_col_hdr_null"] = df["tbl_col_hdr"].isnull()
        ...
        for page,line_row,line_num in pages.groups:
            ...
            if (page == 0) and (line_row == 6) and (line_num == 158):
                line.to_csv(r"d:\sdsd.csv",index = False)
            col_hdrs = list(line["tbl_col_hdr"].unique())
            # print(col_hdrs,len(col_hdrs),page,line_row,line_num)
            if len(col_hdrs) == 2:
                # col_hdr_grps = list(col_hdrs.groups)
                nulls = sum([1 if a == "none" else -1 for a in col_hdrs])
                if nulls != 0:
                    continue
                vals = df[(df["page_num"] == page) &
                          (df["line_num"] == line_num) &
                          (df["tbl_col_hdr"] != "none")][line_cols].values.tolist()[0]
                    ...
                    if "igst" in text_to_compare and key == "lblIGSTAmount":
                        if "fz_" + key + "_" + orientation in tmp_result.keys():
                            print("Temp result before assigning",
                                  ...
                                  tmp_score,text,text_to_compare,key)
...
if __name__ == "__main__":
    inp = {
           "documentId":"doc_1653888860961_889a8aaab9a",
           "client_folder":"Pierian",
           "container":"f3cce063-3d5b-4cec-ab48-623a853f859c",
           "page_details":
               [
                   {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1653888860961_889a8aaab9a-0-pre.tiff",
                    "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1653888860961_889a8aaab9a-0-pre.tiff.ocr.json",
                     "document_page_number":"0"}#,
                    # {"image_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1652251728317_99ba8a9baa8-1-pre.tiff",
                    #   "ocr_path":"f3cce063-3d5b-4cec-ab48-623a853f859c/doc_1652251728317_99ba8a9baa8-1-pre.tiff.ocr.json",
