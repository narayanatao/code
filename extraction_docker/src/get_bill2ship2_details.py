...
def containsAddress(df):
    ...
    df_copy = df.copy(deep =True)
    try:
        df["is_address"] = df["line_text"].apply(checkMatch, args = (addressNameList,))
        return df
    except: 
        print(traceback.print_exc())
        return df_copy
...
def get_vendor_buyers_details(DF):
    ...
    try:
        ...
        print("get_vendor_buyers_details :",traceback.print_exc())
        return df_copy
...
if __name__ == "__main__":
    df = pd.read_csv(r"C:\Users\OVE\Desktop\Git\Swigy_instamart\SWIGGY_TEST_INV & MASTERDATA\SwiggyTestPred\doc_1651725418050_baab8aaa99a_pred.csv" ,index_col = False)
    df = get_bill_to_ship_to_details(df,INDIA_ZIP_CODE_RANGE, billToNames_kws, shipToNames_kws)
    df.to_csv("doc_1650879078994_9b8ab9898a9_pred.csv")
